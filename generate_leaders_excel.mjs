...
async function run() {
  const result = await db.execute("SELECT * FROM leaders ORDER BY name ASC");
  ...
  xlsx.utils.book_append_sheet(workbook, worksheet, "Líderes y Numerología");
  
  const tempPath = '/Users/eduarantoniopenasosa/.gemini/antigravity/brain/0fe5c7af-bee7-4ae1-95ab-dd238ddb8f2e/Lideres_Numerologia.xlsx';
  const outPath = '/Users/eduarantoniopenasosa/Downloads/Lideres_Numerologia.xlsx';
  
  xlsx.writeFile(workbook, tempPath);
  console.log('Saved to temp path:', tempPath);
  
  import('fs').then(fs => {
    fs.copyFileSync(tempPath, outPath);
    console.log('Copied to Downloads folder successfully:', outPath);
  });
}
run();
